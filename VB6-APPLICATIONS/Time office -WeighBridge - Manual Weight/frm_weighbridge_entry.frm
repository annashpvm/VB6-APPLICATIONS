...
Private Sub cmb_load_Click()
    ...
    Dim truck As String
     ...
     If load_first_sec_type = "F" Then
        ...
        pst_qry = "select *  from trn_weighbridge_entry where t_wb_vehicle =  '" & truck & "' and t_wb_compcode = " & compcode & " and  t_wb_net_weight = 0 and t_wb_upd = 'N'"
...
Private Sub cmb_vehicle_Click()
        savetype = "Edit"
        pst_qry = "select *  from trn_weighbridge_entry where t_wb_vehicle =  '" & cmb_vehicle.Text & "' and t_wb_compcode = " & compcode & " and  t_wb_upd = 'N'"
        adocmd_mysql.CommandText = pst_qry
        Set adors = adocmd_mysql.Execute
        If adors.RecordCount > 0 Then
            txt_ticketNo.Text = adors("t_wb_ticketno")
            cmb_material.Text = adors("t_wb_item")
            ...
            txt_EmptyWt.Text = adors("t_wb_2nd_weight")
            txt_NetWt.Text = adors("t_wb_net_weight")
...
Private Sub cmd_close_Click()
       ...
       If Trim(txt_ticketNo.Text) = "" Then
          ...
          Exit Sub
       End If
       If Trim(txt_password.Text) = "Close" Then
        adocmd_mysql.ActiveConnection = gen_connection_mysql
         ...
         If pst_ans = vbYes Then
            ...
            pst_qry = "update trn_weighbridge_entry set t_wb_upd = 'C' ,t_wb_1st_weight = 0, t_wb_2nd_weight = 0  Where t_wb_compcode = " & compcode & " And t_wb_year = " & gin_finid & " And t_wb_ticketno = " & Val(txt_ticketNo.Text) & " and t_wb_net_weight = 0 "
            adocmd_mysql.CommandText = pst_qry
            Set adors = adocmd_mysql.Execute
...
Private Sub cmd_Edit_Click()
    ...
    adocmd_mysql.ActiveConnection = gen_connection_mysql
        ...
        cmb_vehicle.Clear
        txt_ticketNo.Text = ""
        pst_qry = "select *  from trn_weighbridge_entry where t_wb_upd = 'N'"
        adocmd_mysql.CommandText = pst_qry
        Set adors = adocmd_mysql.Execute
        If adors.RecordCount > 0 Then
...
''        cmb_vehicle.Clear
''        txt_ticketNo.Text = ""
''        pst_qry = "select *  from trn_weighbridge_entry where t_wb_net_weight = 0 and t_wb_upd = 'N'"
''        adocmd_mysql.CommandText = pst_qry
''        Set adors = adocmd_mysql.Execute
''        If adors.RecordCount > 0 Then
''            For pin_cnt = 1 To adors.RecordCount
...
''    End If
    ...
    End If
        ...
        If Val(txt_NetWt.Text) < 0 Then
        MsgBox ("NET Weight is  Empty . Please check ... ")
        Exit Sub
    End If
      ...
      If savetype = "New" Then
          pst_qry = "insert into trn_weighbridge_entry (t_wb_year, t_wb_compcode, t_wb_ticketno, t_wb_type, t_wb_date, t_wb_vehicle, t_wb_item, t_wb_party, t_wb_area, t_wb_1st_loadtype, t_wb_1st_weight, t_wb_1st_time, t_wb_2nd_loadtype, t_wb_2nd_time, t_wb_2nd_weight, t_wb_net_weight, t_wb_upd) values (" & gin_finid & " ," & compcode & " ," & Val(txt_ticketNo.Text) & " ,'A','" & Format(dt_ticket, "yyyy-MM-dd") & "' , '" & txt_vehicle.Text & "'," _
          & " '" & Left(cmb_material.Text, 39) & " ' , '" & Left(cmb_supplier.Text, 49) & "' , '', 'L' , " & Val(txt_LoadWt.Text) & " ,'" & Format(dt_ticket2.Value, "yyyy-MM-dd HH:MM:SS") & "', 'E' ,'" & Format(dt_ticket2.Value, "yyyy-MM-dd HH:MM:SS") & "', " & Val(txt_EmptyWt.Text) & "," & Val(txt_NetWt.Text) & " ,'N')  "
          adocmd_mysql.CommandText = pst_qry
          Set adors = adocmd_mysql.Execute
       Else
          pst_qry = "update trn_weighbridge_entry set  t_wb_party =  '" & Left(cmb_supplier.Text, 49) & "' , t_wb_item = '" & Left(cmb_material.Text, 39) & "' , t_wb_1st_weight = " & Val(txt_LoadWt.Text) & " , t_wb_2nd_weight = " & Val(txt_EmptyWt.Text) & " , t_wb_net_weight = " & Val(txt_NetWt.Text) & "  where t_wb_year = " & gin_finid & "  and t_wb_ticketno = " & Val(txt_ticketNo.Text) & " and t_wb_ticketno = " & Val(txt_ticketNo.Text) & ""
          adocmd_mysql.CommandText = pst_qry
          Set adors = adocmd_mysql.Execute
       End If
        
''        MsgBox ("First Entry Saved...")
        savemsg = "Maunal Entry Saved...  "
       
        
        savetype = "New"
        Dim pst_ans, qry, gst_repconnect As String
        pst_ans = MsgBox(savemsg + " Do you want to Print the Ticket...  ", vbYesNo)
        If pst_ans = vbYes Then
        
            Cry_rep1.Formulas(0) = "opt = 0"
''            qry = "{trn_weighbridge_entry.t_wb_ticketno}  =  " & txt_ticketNo.Text
...
Private Sub MSComm1_OnComm()
 ...
 Dim fwt As String
    ...
    str1 = Mid(txt_wt_from_serailport.Text, InStr(txt_wt_from_serailport.Text, ":") + 1, 17)
...
Private Sub txt_EmptyWt_Change()
    If Val(txt_LoadWt.Text) > 0 And Val(txt_EmptyWt.Text) > 0 Then
       txt_NetWt.Text = Val(txt_LoadWt.Text) - Val(txt_EmptyWt.Text)
    Else
       txt_NetWt.Text = ""
