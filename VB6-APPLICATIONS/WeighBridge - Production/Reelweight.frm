...
Begin VB.Form ReelWeigt 
   ...
   Begin VB.Frame Frame1 
      ...
      Top             =   1440
      Width           =   14535
      Begin VB.TextBox txt_pendingqty 
         Enabled         =   0   'False
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   9.75
...
Dim varietycode, custcode, codesize, pin_cnt, prodvartycode As Integer
Dim ginfincode As Integer
    
Dim varinch As String
...
Private Sub cmb_variety_Change()
    ...
    prodvartycode = cmb_variety.ItemData(cmb_variety.ListIndex)
MsgBox (varietycode)
    GetVarietyDetails
End Sub
...
Dim adors As New ADODB.Recordset
    ...
    If adors.RecordCount > 0 Then
        ...
        varinch = adors("var_inchcm")
        varsize = adors("var_size2")

        codesize = adors("var_code")
        lblsize.Caption = adors("var_name")
        
        pst_qry = "select  cust_addnlwt,cust_ref,cust_code,var_code, concat(cast(var_size2 as CHAR) ,space(2) ,(case when var_inchcm = 'I' then 'Inch' else 'CM' end),space(2) ,shade_shortname ) as sizecode , ordt_qty , ordt_fin_wt ,(ordt_qty)+(0.500) - ordt_fin_wt pendingqty from trnsal_order_header, trnsal_order_trailer,massal_variety,masprd_variety,massal_customer,massal_shade   " _
               & " where var_shade = shade_shortcode and  ordh_party = cust_code and  ordh_comp_code = ordt_comp_code and ordh_fincode  = ordt_fincode and ordh_sono = ordt_sono and ordt_var_code = var_code and var_grpcode = var_groupcode and ordh_comp_code = " & compcode & "  and ordh_fincode <= " & fincode & "  and var_grpcode = " & varietycode & "  and ordh_sono = " & cmbSONO.Text & " and ordt_var_code = " & codesize & " order by sizecode"
...
''            txtCustomer.Text = adors("cust_ref")
''            custcode = adors("cust_code")
            txt_pendingqty.Text = adors("pendingqty")
            
            If adors("pendingqty") < 0 Then
                save.Enabled = False
                cmd_Save.Enabled = False
                MsgBox ("Finished Qty exceeds the Order Qty.. You Can't SAVE")
            Else
                save.Enabled = True
            ...
            If adors("cust_addnlwt") = "Y" And ((varinch = "I" And varsize > 19.9) Or (varinch = "C" And varsize > 49)) Then
                 addwt = 1
...
Private Sub cmbSONO_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then
       Call soclickEvent
    End If
End Sub
...
Private Sub cmd_getwt_Click()
    ...
    If cmbReelNo.Text = "" Then
        ...
        Exit Sub
    End If
    If Val(lbl_getwt.Caption) > 10 Then
...
Private Sub cmd_Port_Setting_Click()
    Unload Me
    PortSetting.Show
    PortSetting.ZOrder
End Sub
...
''Dim wtnew, str As String
''Dim wtval As Double
''Dim fractionchk As Single
''Dim newwt As Integer
''Dim f As Integer
''f = InStrRev(Text1.Text, "kg")
''If f > 0 Then
''   str = Mid(Text1.Text, 1, f - 1)
''Else
''  str = Text1.Text
''End If
''
''wtnew = Mid(str, 3)
''wtval = CDbl(Val(wtnew))
''fractionchk = wtval - Int(wtval)
''
''MsgBox (wtval)
''MsgBox (fractionchk)
''If fractionchk > 0.5 Then
''  newwt = Round(wtval, 0)
''Else
''  newwt = Int(wtval)
''End If
...
On Error GoTo err_handler:
   '' Text1.Text = "wn01109.6kg"
    ''Text1.Text = "wn00531.7kg"
...
cmb_finyear_roll.Clear
    ...
    If adors.RecordCount > 0 Then
        For pin_cnt = 1 To adors.RecordCount
                 ...
                 txtRollYear.Text = adors("fin_code")
                 adors.MoveNext
        Next
    End If
    adors.Close
...
''
    pst_qry = "select prd_date ,var_desc,var_groupcode from trn_dayprod_roll_details, masprd_variety where prd_compcode =  " & compcode & " and prd_fincode = " & Val(txtRollYear.Text) & "  and " _
    ...
    If adors.RecordCount > 0 Then
        ...
        For pin_cnt = 1 To adors.RecordCount
                 ...
                 prodvartycode = adors("var_groupcode")
                 adors.MoveNext
        Next
    End If
    adors.Close
...
'' ordt_clo_stat = '' and
    pst_qry = "select  ordh_fincode,ordh_sono from trnsal_order_header, trnsal_order_trailer,massal_variety,masprd_variety " _
              & " where  ordh_comp_code = ordt_comp_code and  ordh_fincode  = ordt_fincode and ordh_sono = ordt_sono and  ordt_var_code = var_code and var_grpcode = var_groupcode and    ordh_comp_code = " & compcode & "  and  ordh_fincode <= " & fincode & "   and " _
              & " var_grpcode = " & varietycode & "  and  (ordt_qty + (ordt_qty*0.25)) > ordt_fin_wt  and (ordt_clo_stat = '' or ordt_clo_stat = 'N' ) group by  ordh_fincode,ordh_sono order by ordh_fincode desc, ordh_sono desc"
...
Function datasave()
    ...
    If saveflag = "new" Then
        ...
        pst_qry = "insert into trnsal_finish_stock  (stk_comp_code,stk_finyear,stk_ent_no,stk_ent_date,stk_var_code,stk_sr_no,stk_wt,stk_sono,stk_source,stk_yymm, stk_rollno, stk_shift, stk_first_wt) VALUES ( " & compcode & ", " & fincode & "," & 100 & ",'" & Format(dt_entrydate, "yyyy-MM-dd") & "'," & codesize & " ," & cmbReelNo.Text & ", " & Val(txtWT.Text) & "," & cmbSONO.Text & ",'R' , " & Val(txtMonth.Text) & " , " & Val(cmbRollNo.Text) & " , '" & Left(cmb_shift.Text, 1) & "', " & Val(txtWT.Text) & ")"
        adocmd_mysql.CommandText = pst_qry
        ...
        pst_qry = "update trn_dayprod_roll_details set prd_finprod = prd_finprod +  " & Val(txtWT.Text) / 1000 & "  where prd_rollno = " & Val(cmbRollNo.Text) & "  and prd_date =  '" & Format(dt_proddate, "yyyy-MM-dd") & "'  and prd_variety = " & prodvartycode & " "
        adocmd_mysql.CommandText = pst_qry
        adocmd_mysql.Execute pst_qry
    ...
    Else
        ...
        pst_qry = "update trnsal_finish_stock set stk_sono = " & cmbSONO.Text & " , stk_var_code = " & codesize & " ,stk_wt = " & Val(txtWT.Text) & " ,stk_source = 'R' where stk_sr_no =  '" & cmbReelNo.Text & "' and stk_comp_code = " & compcode & " and stk_finyear = " & fincode & ""
        adocmd_mysql.CommandText = pst_qry
        adocmd_mysql.Execute pst_qry
...
'''
     txt_getwt = Mid(txt_wt_from_serailport.Text, InStr(txt_wt_from_serailport.Text, "wn") + 2, 7)
'''
...
''        Dim wtval As Double
''        Dim wtnew, str As String
''        Dim f, newwt   As Integer
''        Dim fractionchk As Single
        
''        str = txt_wt_from_serailport.Text
''        f = InStrRev(str, "kg")
...
Dim adors As New ADODB.Recordset
    ...
    pst_qry = "select prd_rollno  from trn_dayprod_roll_details where prd_compcode = " & compcode & "  and prd_fincode = " & Val(txtRollYear.Text) & "  and month(prd_date) = " & mm & "  and year(prd_date) = " & yy & "  and prd_roll_status = 'A'  group by prd_rollno  order by prd_rollno desc"
