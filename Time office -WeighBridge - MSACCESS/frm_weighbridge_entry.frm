...
Option Explicit
Dim mwt As Integer
Dim adocmd_msaccess As New ADODB.Command
...
''       cmb_supplier.Enabled = False
    ...
    Dim truck As String
     ...
     If load_first_sec_type = "F" Then
        ...
        adocmd_msaccess.CommandText = pst_qry
        Set adors = adocmd_msaccess.Execute
...
Private Sub cmd_close_Click()
       ...
       If Trim(txt_password.Text) = "Close1" Then
        adocmd_msaccess.ActiveConnection = gen_connection_msaccess
...
Function refresh_data()
    ...
    pst_qry = "select Nz(max(t_wb_ticketno),0)+1 as tikcetno  from trn_weighbridge_entry where t_wb_year = 25  and t_wb_compcode = 1"
    
    pst_qry = "SELECT Nz(MAX(t_wb_ticketno), 0) + 1 AS ticketno FROM trn_weighbridge_entry WHERE t_wb_year = 25 AND t_wb_compcode = 1"
    
''   pst_qry = "SELECT t_wb_ticketno as ticketno FROM trn_weighbridge_entry WHERE t_wb_year = 25 AND t_wb_compcode = 1"
    
pst_qry = "SELECT IIf(IsNull(MAX(t_wb_ticketno)), 0, MAX(t_wb_ticketno)) + 1 AS ticketno " & _
          "FROM trn_weighbridge_entry WHERE  t_wb_type = 'W' and t_wb_year = " & gin_finid & " AND t_wb_compcode = " & compcode
          

    adocmd_msaccess.CommandText = pst_qry
    Set adors = adocmd_msaccess.Execute
    If adors.RecordCount > 0 Then
         txt_ticketNo.Text = adors("ticketno")
    End If
...
''      End If
      ...
      If wbtype = "W" Then
         pst_qry = "select IIf(IsNull(MAX(t_wb_ticketno)), 0, MAX(t_wb_ticketno)) + 1 AS ticketno  from trn_weighbridge_entry where t_wb_type = 'W' and t_wb_year = " & gin_finid & "  and t_wb_compcode = " & compcode
         adocmd_msaccess.CommandText = pst_qry
         Set adors = adocmd_msaccess.Execute
         If adors.RecordCount > 0 Then
            txt_ticketNo.Text = adors("ticketno")
         End If
      Else
         pst_qry = "select IIf(IsNull(MAX(t_wb_ticketno)), 0, MAX(t_wb_ticketno)) + 1 AS ticketno from trn_weighbridge_entry where t_wb_type = 'Z' and t_wb_year = " & gin_finid & "  and t_wb_compcode = " & compcode & "  and t_wb_date = #" & Format(dt_ticket, "MM-dd-yyyy") & "#"
         adocmd_msaccess.CommandText = pst_qry
         Set adors = adocmd_msaccess.Execute
         If adors("ticketno") = 1 Then
            txt_ticketNo.Text = Trim(dmy) + "01"
         Else
           txt_ticketNo.Text = adors("ticketno")
         End If
      ...
      pst_qry = "insert into trn_weighbridge_entry (t_wb_year, t_wb_compcode, t_wb_ticketno, t_wb_date, t_wb_vehicle," _
        & " t_wb_item, t_wb_party, t_wb_area, t_wb_1st_loadtype, t_wb_1st_weight, t_wb_1st_time ,t_wb_type,t_wb_upd) values ( " & gin_finid & " ," & compcode & " ," & Val(txt_ticketNo.Text) & " ,'" & Format(dt_ticket, "yyyy-MM-dd") & "' , '" & truck & "'," _
        & " '" & Trim(Left(cmb_material.Text, 39)) & " ' , '" & Left(cmb_supplier.Text, 49) & "' , '', '" & wt_type & "' , " & first_sec_wt & " ,'" & Format(dt_ticket2.Value, "yyyy-MM-dd HH:MM:SS") & "','" & wbtype & "','N')  "
        adocmd_msaccess.CommandText = pst_qry
        Set adors = adocmd_msaccess.Execute
...
''Modified on 27/05/2024
    ...
    Dim pst_ans, gst_repconnect, qry As String
        ...
        If pst_ans = vbYes Then
           ...
           gst_repconnect = "dsn=access"
            
''            Cry_rep1.PrinterSelect
            Cry_rep1.ReportFileName = "d:\wbreports\Accessweigh_slip.rpt"
        ...
        pst_qry = "select IIf(IsNull(MAX(t_wb_ticketno)), 0, MAX(t_wb_ticketno)) + 1 AS ticketno from trn_weighbridge_entry where t_wb_year = " & gin_finid & "  and t_wb_compcode = " & compcode
        adocmd_msaccess.CommandText = pst_qry
        Set adors = adocmd_msaccess.Execute
        If adors.RecordCount > 0 Then
            txt_ticketNo.Text = adors("ticketno")
...
''    pst_qry = "select ifnull(max(t_wb_ticketno),0)+1 as tikcetno  from trn_weighbridge_entry where t_wb_type = "W" and t_wb_year = " & yy & "  and t_wb_compcode = " & compcode
''    adocmd_msaccess.CommandText = pst_qry
''    Set adors = adocmd_msaccess.Execute
''    If adors.RecordCount > 0 Then
''         txt_ticketNo.Text = adors("tikcetno")
...
''    pst_qry = "select item_code, item_name, item_group , '99' as ordprint from mas_wb_item "
