...
If fst_save = "NEW" Then
    ...
    pst_qry = "insert into mas_wb_item values(" & no & ",'" & Trim(UCase(cmb_item_name.Text)) & "'," & cmb_item_group.ItemData(cmb_item_group.ListIndex) & ")"
    adocmd_msaccess.CommandText = pst_qry
...
ElseIf fst_save = "EDIT" Then
    ...
    pst_qry = "update mas_wb_item set item_name='" & Trim(UCase(txt_itemname.Text)) & "' , item_group ='" & cmb_item_group.ItemData(cmb_item_group.ListIndex) & "'  where item_name='" & UCase(cmb_item_name.Text) & "'"
    adocmd_msaccess.CommandText = pst_qry
    adocmd_msaccess.Execute pst_qry
...
Function fillgrid()
   ...
   Dim pin_cnt As Integer
    adocmd_msaccess.ActiveConnection = gen_connection_msaccess
    pst_qry = "select * from mas_wb_item ,  mas_wb_itemgroup where item_group = item_grpcode order by item_name"
    adocmd_msaccess.CommandText = pst_qry
    Set adors = adocmd_msaccess.Execute
    If adors.RecordCount > 0 Then
        For pin_cnt = 1 To adors.RecordCount
