...
Public Sub gen_dbconnection()
    ...
    pst_conn = Space(150)
    'pst_ret = GetPrivateProfileString("PASSWORD", "DB", "Default Value", pst_conn, 255, "\\192.168.3.5\Reports\Ini\dbpass.ini")
    'gst_password = Trim(Left(Trim(pst_conn), Len(Trim(pst_conn)) - 1))
    
    
    '' strcnn = "Provider=SQLOLEDB.1;Persist Security Info=False;User ID=sa;pwd=serdat;Initial Catalog=shvpm;Data Source=10.0.0.252"
    
    
    'strcnn_mysql = "DRIVER={MySQL ODBC 3.51 Driver};SERVER=192.168.11.16;DATABASE=dpm;USER=root;PASSWORD=mysql"
