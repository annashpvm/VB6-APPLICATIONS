...
Declare Function FindItem Lib "user32" Alias "SendMessageA" (ByVal hwnd As Long, ByVal wMsg As Long, ByVal wParam As Long, ByVal lParam As String) As Long
...
Global control_lock As Integer
Global data_source As String
Global pwchk As Integer
Global optchk As Integer
Global findyear As Integer
Global fyear As String
Global gdt_finsdate, gdt_finedate As Date
Global uname As String
Global arrearamt As Double
Global poweruser, adminpw As Integer
...
Public loc, loc2, casual As String
...
''    pay = "Provider=SQLOLEDB.1;Password=serdat;Persist Security Info=True;User ID=sa;DATABASE=shvpm;Data Source=10.0.0.252"
    pay = "DRIVER={MySQL ODBC 3.51 Driver}; SERVER=10.0.0.251; PORT = 3306; DATABASE=shvpmpayroll; USER=root; PASSWORD=P@ssw0rD; OPTION=3; CHARSET = UTF8; SOCKET = MYSQL"
    frm_login.Show
End Sub
...
Select Case errno
       Case -2147467259
            Pst_Msg = "Invalid DSN/MDB/DB Name OR ESSL software to be closed and try"
       Case -2147217900
...
Public Function attndays_Chk(key As Integer, txt_value As Variant, tot_len As Integer, Optional i As Integer, Optional d As Integer) As Integer
    ...
    Else
        If key > Asc("2") And Len(txt_value) = 0 Then txt_value = "00.00"
        If key > Asc("7") And Len(txt_value) = 1 And Left(txt_value, 1) = "2" Then txt_value = "20.00"
        If InStr(1, txt_value, ".") = 0 Then
            If Len(txt_value) = i Then
                attndays_Chk = 46
                Exit Function
...
Function find_present_status(ecode As Long) As String
    ...
    Exit Function
End Function
Function find_arrear(ecode As Long, fyear As Integer, fmonth As Integer, fcompany As Integer)
''    sql = "Select * from arrear_entry where e_month = " & fmonth & " and e_year = " & fyear & " and e_company = " & fcompany & " and e_empcode  = " & ecode & ""
''    Set paydb2 = New ADODB.Connection
''    Set payrs2 = New ADODB.Recordset
''    paydb2.Open pay
''    payrs2.Open sql, paydb2, adOpenDynamic, adLockOptimistic
...
Public Function find_index_item_data(pcmb_source As ComboBox, pst_search As String)
    ...
    For pin_idx = 0 To pcmb_source.ListCount - 1
       If pcmb_source.ItemData(pin_idx) = pst_search Then
