...
Global gcn_shvpm As New ADODB.Connection
...
Global poweruser, userrights As Integer
...
Function find_empdetails(data_code As String) As String
    sql = ("Select * from emp_mas where emp_code = '" & data_code & "' and emp_company = '" & company_code & "'")
    Set paydb2 = New ADODB.Connection
    Set payrs2 = New ADODB.Recordset
    paydb2.Open pay
    payrs2.Open sql, paydb2, adOpenDynamic, adLockOptimistic
...
Public Function attndays_Chk(key As Integer, txt_value As Variant, tot_len As Integer, Optional i As Integer, Optional d As Integer) As Integer
    ...
    If i = 0 Then
        ...
        End If
    Else
        If key > Asc("3") And Len(txt_value) = 0 Then txt_value = "00.00"
        If key > Asc("7") And Len(txt_value) = 1 And Left(txt_value, 1) = "2" And Left(txt_value, 1) = "3" Then txt_value = "20.00"
        If InStr(1, txt_value, ".") = 0 Then
            If Len(txt_value) = i Then
                attndays_Chk = 46
                Exit Function
