...
Begin VB.Form salary_statement_worker 
   Caption         =   "WORKER SALARY STATEMENTS"
   ClientHeight    =   8940
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   15240
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Frame Frame2 
         ...
         Top             =   360
         Width           =   7320
         Begin VB.Frame Frame_stoppage 
            Caption         =   "Percentage of Exgratia"
            Height          =   735
            Left            =   4080
            TabIndex        =   28
            Top             =   2760
            ...
            End
         End
         Begin VB.OptionButton opt_stoppage 
            Caption         =   "Stoppage Wages @ % Basis"
            Height          =   285
            Left            =   240
            TabIndex        =   27
            Top             =   3120
            Width           =   3615
         End
         Begin VB.OptionButton opt_worker_all 
            Caption         =   "ALL WORKER - for PRESENT DAYS - Format 2"
            Height          =   285
            Left            =   240
            TabIndex        =   25
            Top             =   2640
      ...
      Begin VB.Frame Frame3 
         ...
         Begin VB.CommandButton PROCESS 
            ...
            Height          =   825
            Left            =   120
            Picture         =   "salary_statement_worker.frx":0000
            Style           =   1  'Graphical
            TabIndex        =   14
            Top             =   120
            Width           =   720
         ...
         Begin VB.CommandButton EXIT 
            ...
            Height          =   825
            Left            =   840
            Picture         =   "salary_statement_worker.frx":066A
            Style           =   1  'Graphical
            TabIndex        =   13
            Top             =   120
            Width           =   720
   ...
   Begin VB.Label Label4 
      Caption         =   " THIS REPORT IS FROM  JUNE-2015 AND AFTER MONTHS ONLY"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "salary_statement_worker"
...
Private Sub opt_stoppage_Click()
    Frame_stoppage.Visible = True
End Sub
...
Private Sub PROCESS_Click()
   ...
   If opt_stoppage.Value = True Then disname = "WORKER Ex-GRATIA FOR THE MONTH OF "
...
''      ds = " and {emp_mas.emp_workplace} = 'MILL' and {emp_mas.emp_classification} = 'B'"
   ...
   ElseIf opt_wp_pdays = True Then
        cry_rep1.ReportFileName = "\\10.0.0.252\vbcryrep\PAYROLL\salary_statement_worker_forlayoff.rpt"
        cry_rep1.Formulas(5) = ("cond = 0")
        cry_rep1.Formulas(6) = ("speriod = 'FOR WORKING DAYS'")
   ElseIf opt_wt_pdays = True Then
        cry_rep1.ReportFileName = "\\10.0.0.252\vbcryrep\PAYROLL\salary_statement_worker_trainee_forlayoff.rpt"
        cry_rep1.Formulas(5) = ("cond = 0")
        cry_rep1.Formulas(6) = ("speriod = 'FOR WORKING DAYS'")
   ElseIf opt_wp_layoff = True Then
    ...
    ElseIf opt_worker_all = True Then
        cry_rep1.ReportFileName = "\\10.0.0.252\vbcryrep\PAYROLL\salary_statement_worker_presentdays.rpt"
        cry_rep1.Formulas(5) = ("cond = 0")
        cry_rep1.Formulas(6) = ("speriod = 'FOR PRESENT DAYS'")
    ElseIf opt_stoppage.Value = True Then
        cry_rep1.ReportFileName = "\\10.0.0.252\vbcryrep\PAYROLL\worker_exgratia_statement_forstoppage.rpt"
        
        cry_rep1.Formulas(6) = ("calc_per= " & Val(txt_per.Text))
   ...
   ElseIf opt_worker_all.Value = True Then
        cry_rep1.ReplaceSelectionFormula ("{emp_salary.s_year} = " & Val(cmb_year.Text) & " and {emp_salary.s_month}= " & cmb_month.ItemData(cmb_month.ListIndex) & _
        "and {emp_salary.s_company} = " & company_code & " and ({emp_mas.emp_type} = 2 or {emp_mas.emp_type} = 3 ) and {emp_salary.s_salarydays} > 0  " & ds & "")
   ElseIf opt_stoppage.Value = True Then
        cry_rep1.ReplaceSelectionFormula ("{emp_salary.s_year} = " & Val(cmb_year.Text) & " and {emp_salary.s_month}= " & cmb_month.ItemData(cmb_month.ListIndex) & _
        "and {emp_salary.s_company} = " & company_code & " and ({emp_mas.emp_type} = 2 or {emp_mas.emp_type} = 3 )")
   
        pst_qry = "{emp_salary.s_year} = " & Val(cmb_year.Text) & " and {emp_salary.s_month}= " & cmb_month.ItemData(cmb_month.ListIndex) & " and {emp_salary.s_company} = " & company_code & " and ({emp_mas.emp_type} = 2 or {emp_mas.emp_type} = 3 )"
   Else
        cry_rep1.ReplaceSelectionFormula ("{emp_salary.s_year} = " & Val(cmb_year.Text) & " and {emp_salary.s_month}= " & cmb_month.ItemData(cmb_month.ListIndex) & _
